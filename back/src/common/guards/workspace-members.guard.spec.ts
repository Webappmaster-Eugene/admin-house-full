...
import { WorkspaceMembersGuard } from './workspace-members.guard';
...
function buildGuard(
  ...
  return new WorkspaceMembersGuard(configService, userService as IUserService, workspaceService as IWorkspaceService, logger as ILogger);
}

describe('WorkspaceMembersGuard', () => {
  ...
  it('should throw 400 BadRequest when workspaceId is the literal string "undefined"', async () => {
    const guard = buildGuard();
    const context = buildMockExecutionContext({
  ...
  it('should return true for ADMIN regardless of workspace membership', async () => {
  ...
  it('should return true for MANAGER who is member of the workspace', async () => {
    const managerMember: MinimalUser = {
      uuid: USER_UUID,
  ...
  it('should return true for MANAGER who is creator of the workspace', async () => {
    const managerCreator: MinimalUser = {
      uuid: USER_UUID,
      roles: [{ uuid: 'role-m', idRole: ROLE_IDS.MANAGER_ROLE_ID, name: 'MANAGER' }],
      memberOfWorkspaces: [],
  ...
  it('should throw 403 for user who is neither member nor creator of workspace', async () => {
  ...
  it('should throw 403 when user from JWT is not found in DB', async () => {
