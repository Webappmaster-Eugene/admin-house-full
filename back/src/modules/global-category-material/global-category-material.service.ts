...
@Injectable()
export class GlobalCategoryMaterialService implements IGlobalCategoryMaterialService {
  constructor(
  ...
  async getById(
    ...
    const globalCategoryMaterial: GlobalCategoryMaterialEntity = await this.queryBus.execute(
      new GetGlobalCategoryMaterialByIdQuery(globalCategoryMaterialId),
    );
    return new InternalResponse(globalCategoryMaterial);
  }

  async getAll(): Promise<UniversalInternalResponse<GlobalCategoryMaterialEntity[]>> {
    const globalCategoryMaterials: GlobalCategoryMaterialEntity[] = await this.queryBus.execute(new GetAllGlobalCategoryMaterialsQuery());
  ...
  async create(dto: GlobalCategoryMaterialCreateRequestDto): Promise<UniversalInternalResponse<GlobalCategoryMaterialEntity>> {
    ...
    return new InternalResponse(createdGlobalCategoryMaterial);
  }
  ...
  async updateById(
    ...
    const updatedGlobalCategoryMaterial: GlobalCategoryMaterialEntity = await this.commandBus.execute<
      UpdateGlobalCategoryMaterialCommand,
      GlobalCategoryMaterialEntity
    >(new UpdateGlobalCategoryMaterialCommand(globalCategoryMaterialId, dto));

    return new InternalResponse(updatedGlobalCategoryMaterial);
  }

  async deleteById(
    globalCategoryMaterialId: EntityUrlParamCommand.RequestUuidParam,
  ): Promise<UniversalInternalResponse<GlobalCategoryMaterialEntity>> {
    const deletedGlobalCategoryMaterial: GlobalCategoryMaterialEntity = await this.commandBus.execute<
      DeleteGlobalCategoryMaterialCommand,
      GlobalCategoryMaterialEntity
    >(new DeleteGlobalCategoryMaterialCommand(globalCategoryMaterialId));

    return new InternalResponse(deletedGlobalCategoryMaterial);
  }
}
