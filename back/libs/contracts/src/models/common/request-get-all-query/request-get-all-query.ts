...
export const RequestGetAllQuerySchema = z
  .object({
    take: z.number().multipleOf(5).nonnegative().max(QUANTITY_LIMIT_QUERY.TAKE_MAX_LIMIT).optional().default(QUANTITY_LIMIT_QUERY.TAKE_5),
    skip: z.number().nonnegative().optional().default(0),
  })
  .refine(
    data => {
      const takeValue = data.take || QUANTITY_LIMIT_QUERY.TAKE_5;
