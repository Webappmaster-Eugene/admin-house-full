...
let AutomapperModule = (AutomapperModule_1 = class AutomapperModule {
  ...
  static createProviders(mapperOptions, globalOptions) {
    return mapperOptions.map(({ name, namingConventions, strategyInitializer, errorHandler }) => {
...
function createMapPipe(from, to, options) {
  const { isArray, mapperName, transformedMapOptions } = getTransformOptions(options);
  ...
  MixinMapPipe = __decorate(
    [__param(0, Optional()), __param(0, InjectMapper(mapperName)), __metadata('design:paramtypes', [Object])],
    MixinMapPipe,
  );
...
function createMapInterceptor(from, to, options) {
  ...
  MixinMapInterceptor = __decorate(
    [__param(0, Optional()), __param(0, InjectMapper(mapperName)), __metadata('design:paramtypes', [Object])],
    MixinMapInterceptor,
  );
